...
#create clauses for every 5 points
    ...
    for i2 in range(2):
        ...
        for i3 in range(2):
            ...
            for i4 in range(2):
                ...
                cnf.append(buildcase(p1,p5,above,below))
                if (last4): above.pop()
                else: below.pop()
            if (last3): above.pop()
            else: below.pop()
        ...
        for p3 in range(p2+1,n+1):
            for p4 in range(p3+1,n+1):
                for p5 in range(p4+1,n+1):
                    iterative(p1,p2,p3,p4,p5)


solver = Glucose42(with_proof=True)
